...
2. Download `SubsTrack-0.1.8-sideload.apk`.
...
Android 7.0 or newer is required. Version 0.1.8 upgrades an existing 0.1.4–0.1.7 installation without removing the app. It repairs local database migration for existing installations; upgrading from 0.1.7 and saving a billing date were verified on a physical Android device. Do not clear app data before upgrading. The web version is available at [substrack.top](https://substrack.top).

The APK checksum is attached to the release and stored in `SubsTrack-0.1.8-sideload.apk.sha256`.
